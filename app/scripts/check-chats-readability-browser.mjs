...
const measure=`(()=>{
  ...
  const rows=[...b.querySelectorAll('.project-session')].map(e=>{
    const title=e.querySelector('span')||e,del=e.parentElement.querySelector('.session-trash');
    const dr=del?rect(del):{left:0,right:0,top:0,bottom:0,width:0,height:0},s=del?getComputedStyle(del):{backgroundColor:'rgba(0, 0, 0, 0)',borderTopWidth:'0'};
    const hit=(el,x,y)=>{const h=document.elementFromPoint(x,y);return el===h||el.contains(h);};
    const visible=!!del&&dr.top>=br.top&&dr.bottom<=br.bottom;
    return {id:e.dataset.sessionId,row:rect(e.parentElement),title:rect(title),titleText:title.textContent,open:rect(e),action:dr,disabled:!del||del.disabled,actionBackground:s.backgroundColor,actionBorder:parseFloat(s.borderTopWidth),hit:!visible||[[.5,.5],[.25,.5],[.75,.5],[.5,.25],[.5,.75]].every(([x,y])=>hit(del,dr.left+dr.width*x,dr.top+dr.height*y))};
  });
  return {width:innerWidth,height:innerHeight,root:rect(document.querySelector('#root')),body:br,scrollTop:b.scrollTop,scrollHeight:b.scrollHeight,header:rect(document.querySelector('header')),tabs:rect(document.querySelector('.shell-tabbar')),rows,overflow:document.documentElement.scrollWidth>innerWidth};
})()`;
try {
  await browser.start();
    ...
    for(const width of [320,390,430]) {
      ...
      await check(`${tag}: readable project session titles and 44px row targets`,async()=>{
        ...
        for(const r of top.rows) {
          ...
          assert.ok(r.action.width>=44&&r.action.height>=44&&r.action.right<=width,q(r));
          assert.equal(r.actionBackground,'rgba(0, 0, 0, 0)');assert.equal(r.actionBorder,0);
        }
      });
      await browser.evaluate(`(()=>{const b=document.querySelector('.project-browser');b.scrollTop=Math.min(120,Math.max(0,b.scrollHeight-b.clientHeight));})()`);await browser.settle();await j.shot(`${tag}-scrolled`);
  ...
  await check('session delete confirmation can be cancelled',async()=>{
    const deletable=await browser.evaluate(`[...document.querySelectorAll('.session-trash')].find(e=>!e.disabled)?.getAttribute('aria-label')`);
    assert.ok(deletable);await j.tap(deletable);await j.text('Delete this session?');await j.tap('Cancel','dialog');
