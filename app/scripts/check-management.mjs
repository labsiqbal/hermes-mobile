...
async function browserCheck() {
  const fixture = `
    ...
    const profileRow=name=>[...document.querySelectorAll('.manage-row')].find(row=>row.querySelector('strong')?.textContent===name);
    const chooseProfileRow=async name=>{const row=profileRow(name);check(row,'missing profile row '+name);row.click();await tick();check(profileRow(name).getAttribute('aria-pressed')==='true','exact profile row selected');};
    const content=()=>document.querySelector('.manage').textContent;
    const writes=()=>calls.filter(c=>c.method==='profiles.configure').length;
    async function run(){
      for(let i=0;i<20&&!button('Profiles2 profiles reported by this gateway');i++)await tick();
      check(!document.querySelector('header,main,nav'), 'Manage must not add an application shell');
      check(!document.querySelector('select'), 'hub must not show an irrelevant profile selector');
      await click('CapabilitiesInstalled skills, toolsets and MCP configuration');
      check(document.querySelector('select').value==='', 'must not silently select default profile');
      check(calls.length===0&&requests.length===0,'unselected profile must not request profile data');
      await click('Back to Manage');
      await click('Profiles2 profiles reported by this gateway');
      check(!document.querySelector('select'),'Profiles must use its roster, not a redundant selector');
      check(!document.querySelector('.manage-row[aria-pressed="true"],textarea'),'Profiles must not silently select default profile');
      await chooseProfileRow('research');
      ...
      await chooseProfileRow('studio');check(document.querySelector('textarea').value==='Studio only','draft cannot leak across profiles');
      await chooseProfileRow('research');check(document.querySelector('textarea').value==='Reviewed description','research readback retained');
      await click('Back to Manage');
      ...
      check(!document.querySelector('select'),'new gateway hub must not show irrelevant profile selector');
      check(!content().includes('Reviewed description'),'previous gateway details must not leak');
      await click('Profiles2 profiles reported by this gateway');
      check(!document.querySelector('.manage-row[aria-pressed="true"],textarea'),'new gateway must clear management profile');
      root.render(<Manage {...props}/>);await tick();
      check(profileRow('research')?.getAttribute('aria-pressed')==='true'&&profileRow('studio')?.getAttribute('aria-pressed')==='false','returning gateway restores only its selected profile');
