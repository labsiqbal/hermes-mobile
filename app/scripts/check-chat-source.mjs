...
try {
 ...
 await test('new-session folder accepts root and trailing slash but rejects sensitive paths',async()=>{for(const path of ['/','/workspace/project','/workspace/project/','/home/operator/workspace/.worktrees/project/feature'])assert.equal(safeServerFolder(path),true);for(const path of ['','relative','/workspace/../secret','/workspace/%2e%2e/secret','/home/operator/.ssh','/workspace/key.pem'])assert.equal(safeServerFolder(path),false);});
