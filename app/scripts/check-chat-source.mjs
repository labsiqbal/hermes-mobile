...
try {
 ...
 await test('new-session folder accepts only explicit safe absolute server paths',async()=>{for(const path of ['/workspace/project','/home/iqbal/workspace/.worktrees/project/feature'])assert.equal(safeServerFolder(path),true);for(const path of ['','/','relative','/workspace/../secret','/workspace/%2e%2e/secret'])assert.equal(safeServerFolder(path),false);});
