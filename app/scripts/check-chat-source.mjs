...
try {
 ...
 await test('completion adapter rejects malformed rows before UI mapping',async()=>{const real=new HermesConnection({url:'https://fixture.invalid',username:'fixture',password:'fictional'});const wire=[];real.rpc=async(method,params)=>{wire.push({method,params});return{items:[{text:'@folder:project'},{text:'plain-file'},{text:42},null]};};assert.deepEqual(await real.completePath('@folder:pro','/workspace'),[{text:'@folder:project'}]);assert.deepEqual(wire,[{method:'complete.path',params:{word:'@folder:pro',cwd:'/workspace'}}]);});
