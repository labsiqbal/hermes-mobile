...
- Resume chats, read streaming replies and tool activity, attach files, and respond to explicit command approvals. Composer supports `/exit` (close idle one-to-one Mobile runtime; chat remains resumable) and `/model` (open session model sheet). Model and reasoning controls also sit inside composer's bottom row.
- Browse sessions in a dark folder tree under Chats, or the persistent desktop sidebar. Add a server-folder bookmark, create a session directly in that folder, and reorder projects by dragging or using Alt+ArrowUp/ArrowDown. Folder bookmarks and ordering are local to this browser and connection; reordering never moves folders on disk. Bots contains bot identities, their threads and bot groups.
- Project menus support local pinning, editing, connection color and hiding. Touch-hold a heading to drag; short swipes scroll. Keyboard reordering uses Alt+ArrowUp/ArrowDown. Session trash controls retain explicit confirmation and gateway ownership checks.
- Type `/` in chat or bot composers to search the gateway's full `commands.catalog`, including skills and custom commands. `/exit` and `/model` retain their local behavior. Argument-free `/help`, `/status`, `/version`, `/whoami`, `/usage` and `/commands` use the session/profile-bound information adapter. Other catalogue entries are marked Desktop and are never silently submitted as prompts; full Desktop command execution parity is not claimed.
- Add bots from the Bots roster using the gateway's `profiles.create`: choose a name, description, instructions and source profile. Provider credential reuse is explicit; opting out requires a fresh profile. Channel credentials and shell aliases are not cloned/created. Creation refreshes the roster without creating sessions or sending prompts; unsupported gateways and uncertain acknowledgments are reported visibly.
